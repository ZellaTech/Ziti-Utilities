...
$script:AutoUpdate		= "false" # Instructs the program to check for an update to itself from the specified server (true=try to update | false=ignore).
...
$script:ZDERVer		= "2.5.1.1" # ZITI Desktop Edge (Win) version to target from repos (AUTO=find automatically | [X.XX.XX=target this version]).
